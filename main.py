...
async def start_handler(bot: Client, m: Message):
    ...
    await m.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("Developer - @searchingshiv", url="https://t.me/searchingshiv")],
...
async def callback_handlers(bot: Client, cb: CallbackQuery):
    if "mergeNow" in cb.data:
        ...
        await cb.message.edit(
            text=Config.START_TEXT,
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Developer - @searchingshiv", url="https://t.me/searchinshiv"),
            disable_web_page_preview=True
        )
        ...
        if (QueueDB.get(cb.from_user.id, None) is None) or (QueueDB.get(cb.from_user.id) == []):
            await cb.answer("Sorry Unkil, Your Queue is Empty!", show_alert=True)
            return
        merged_vid_path = f"{Config.DOWN_PATH}/{str(cb.from_user.id)}/[@searchingshiv
                                               
                                               
                                               
                                               ]_Merged.{FormtDB.get(cb.from_user.id).lower()}"
